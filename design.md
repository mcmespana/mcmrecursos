...
## 7. Esta app: Banco de Recursos

**Qué es visualmente:** una **biblioteca luminosa**. Mucho aire, tarjetas como fichas, el
teal "consolación" (agua, calma) como identidad y el ámbar (`--warm`) como chispa de la capa
social — estrellas, corazones, "lo más usado". El contenido —las miniaturas de los recursos—
es el color protagonista; el chrome se aparta. Personalidad en la portada y las estanterías;
sobriedad quirúrgica en filtros, tabla y ficha.

**Es la referencia del sistema** para portada editorial, movimiento, estados de acción y
patrón de deshacer. Cuando otra app dude en esas cuatro cosas, mira aquí.

**Dónde está cada cosa**

| Qué | Dónde |
|---|---|
| **Documento de diseño largo** | `docs/04-diseno.md` — sigue siendo la fuente canónica de esta app. Este `design.md` es el marco común; el 04 tiene el detalle |
| Reflexión de UI/UX con hallazgos numerados | `docs/06-reflexion-uiux.md` |
| Tokens, curvas, `.toque`, view transitions | `app/src/app.css` |
| Primitivas (shadcn-svelte / Bits UI) | `app/src/lib/components/ui/` |
| Componentes propios | `app/src/lib/components/` |
| Deshacer | `app/src/lib/deshacer.ts` |
| View transition tarjeta → ficha | `app/src/lib/transicion.svelte.ts` |
| Planes de diseño pendientes | `design-plans/` |

**Decisiones propias de esta app, que se respetan:**

- **Color por familia de tipo** (Sesiones=teal, Actividades=verde, Celebración=violeta,
  Audiovisual=ámbar, Documentos=gris-azul). **El color sigue a la entidad, nunca a la
  posición.**
- **Ficha en hoja lateral** (escritorio) o **bottom sheet casi a pantalla completa** (móvil)
  **sobre** la búsqueda, sin destruir su estado. Navegación ←/→ entre resultados.
- **La firma de la app es el reordenado**: `animate:flip` ~220 ms al refiltrar, y la
  miniatura que viaja de la tarjeta a la ficha con View Transition (240 ms). El
  `view-transition-name` tiene que ser único **en cada instante**: se pasa de la tarjeta a la
  ficha dentro de la propia transición y se retira al acabar.
- **`<Button cargando textoCargando="Guardando…">` y `<Button hecho>`** son la
  implementación de referencia de la §3.5. Úsalos en vez de montar spinners a mano.
- **Escritorio usa el ancho**: hasta 4–5 columnas de galería. Nada de tubo centrado.
- **El estado vacío del catálogo prueba a quitar cada filtro por separado** y ofrece los que
  devuelven algo (§3.5). Es el mejor vacío que tenemos.
- **`--warm` es solo la capa social.** No lo uses como segundo acento de la interfaz.

**El dato que ordena las prioridades** (`docs/06-reflexion-uiux.md`): el banco tiene **siete
recursos públicos**. La app se ve casi siempre en su estado más pobre, así que **cualquier
decisión de diseño que solo luzca con doscientos recursos hoy resta**. Y el cuello de botella
no es *encontrar*, es *tener*: lo que facilita aportar material vale más que lo que mejora el
buscador.

**Lo que hay que saber para no tropezar:**

- `docs/04-diseno.md` §4 describe **la dirección completa**, y dos piezas **no están
  construidas** y van marcadas «🔜 no construido»: las estanterías editoriales de la portada
  y el drawer de facetas en móvil. No las des por hechas.
- Detalle de LayerChart: los objetos de `props={{ highlight: { lines: … } }}` **se descartan
  si no llevan `class`**. Por eso la regla del crosshair se fuerza con
  `class="… [stroke-dasharray:none]"`.
- **Svelte 5 con runes**, modo forzado. Nada de `export let` ni `$:` en componentes nuevos.
